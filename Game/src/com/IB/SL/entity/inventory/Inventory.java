		...
		//for (int i = 0; i < equipment.length; i++) {
			...
			try {
				...
				System.out.println();*/
					...
					if (slotCheck != null) {
						font8x8.render(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, -3, slotCheck.getName().toUpperCase(), screen, false, true);
...
//	}
	...
	public void renderTabITEMS(Screen screen) {
		...
		font8x8.render(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, -3, currentItemName, screen, false, true);
	...
	public void renderEffects(Screen screen, Effect[] effects) {
		...
		for (int i = 0; i < effects.length; i++) {
			...
			font8x8.render(20, 46 + (i * 8), -2, 0xff000000, effects[i].getName(), screen, false, true, 1);			
			}
		}
	}
	...
	public void renderTabQUESTS(Screen screen, Quest[] quests) {
		...
		questDesc = "N/A";
			...
			try {
				questDesc = detectQuestButton(quests).getDesc();
	...
	private Quest detectQuestButton(Quest[] quests) {
		if (Game.getGame().getPlayer().gui.checkBounds(87, 51, 38, 8, true, true)) {
			return quests[0];
		} else {
			return null;
		}
	...
	public void SwitchTab(TAB tab, boolean isButton) {
		...
		if (moving != null) add(moving); moving = null;
	}
