...
public class BlinkSpell extends Ability{
	...
	public boolean use(Projectile p, Ability ability, Mob origin) {
		...
		if(Game.getGame().getLevel().returnTileXY(Game.getGame().getLevel().tile, Screen.xo, Screen.yo).solid() == false || Game.getGame().devModeOn) {
			Game.getGame().getLevel().add(new WallParticleSpawner((int) origin.getX(), (int) origin.getY(), 20, 200, Game.getGame().getLevel()));
			Game.getGame().getPlayer().x = Screen.xo << Game.TILE_BIT_SHIFT;
			Game.getGame().getPlayer().y = Screen.yo << Game.TILE_BIT_SHIFT;
			Game.getGame().getLevel().add(new WallParticleSpawner((int) origin.getX(), (int) origin.getY(), 20, 200, Game.getGame().getLevel()));
