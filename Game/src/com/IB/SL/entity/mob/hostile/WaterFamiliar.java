...
public class WaterFamiliar extends Mob {
	...
	private AnimatedSprite down = new AnimatedSprite(SpriteSheet.WElemental_down,
			16, 16, 0);
	private AnimatedSprite up = new AnimatedSprite(SpriteSheet.WElemental_up, 16,
			16, 0);
	private AnimatedSprite left = new AnimatedSprite(SpriteSheet.WElemental_left,
			16, 16, 0);
	private AnimatedSprite right = new AnimatedSprite(SpriteSheet.WElemental_right, 16, 16, 0);
	...
	public void stab() {
		try {
			...
			for (int i = 0; i < ent.size(); i++) {
				Game.getGame().getLevel().damage((int)x, (int)y, (Mob)ent.get(0), 0, 1 + (Game.getGame().getPlayer().stat_base_MDF * 0.15), name, 0);
					}
				}
			}
		} catch (Exception e) {
