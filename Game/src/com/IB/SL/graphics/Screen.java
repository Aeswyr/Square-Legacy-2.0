	...
	public void renderSheet(int xp, int yp, SpriteSheet sheet, boolean fixed) {
		...
		for (int y = 0; y < sheet.SPRITE_HEIGHT; y++) {
			...
			for (int x = 0; x < sheet.SPRITE_WIDTH; x++) {
				int xa = x + xp;
				if (xa < 0 || xa >= width || ya < 0 || ya >= height) continue;
				int colour = sheet.pixels[x + y * sheet.SPRITE_WIDTH];
				if (colour != ALPHA_COL) {
	...
	public void renderSprite(int xp, int yp, Sprite sprite, boolean fixed) {
		...
		for (int y = 0; y < sprite.getHeight(); y++) {
			...
			for (int x = 0; x < sprite.getWidth(); x++) {
				...
				if (colour != ALPHA_COL) { // if the colour is not the pink
				     pixels[(int) (xa + ya * width)] = colour;
				}
			}
		}		
	}
...
public void renderTile(int xp, int yp, Sprite sprite) {
	...
	for (int y = 0; y < sprite.SIZE; y++) {
		...
		for (int x = 0; x < sprite.SIZE; x++) {
			...
			//if (colour != ALPHA_COL) pixels[xa + ya * width] = colour;
				if (colour != ALPHA_COL) {
					pixels[xa + ya * width] = colour;
				    colour = MyColor.changeBrightness(colour, Level.brightness, false);
				    pixels[(int) (xa + ya * width)] = colour;
				}
	}
}
...
public int colSwitch(int col, int tilesx, int tilesy) {
	  ...
	  public void fade(int r, int g, int b) {
		   int col = 0;
			...
			public boolean shakeScreen() { // to use you gotta put an if state in game on 
				...
				try {
					...
					for (int y = 0; y < map.SIZE; y++) {
						...
						for (int x = 0; x < map.SIZE; x++) {
						  ...
						  //if (d <= size) {
							int colour = map.pixels[x + y * map.getWidth()];
							pixels[(int) (xa + ya * width)] = colour;
					          //  }
						}
				...
				/*SpriteSheet sheet = SpriteSheet.minimapDYN;
					for (int y = 0; y < (int)(Game.getGame().getPlayer().getY() / 16) + 32; y++) {
						...
						for (int x = 0; x < (int)(Game.getGame().getPlayer().getX() / 16) + 32; x++) {
							...
							int colour = sheet.pixels[x + y * 32];
							if (colour != ALPHA_COL) {
							     pixels[xa + ya * width] = colour;
							}
						}
