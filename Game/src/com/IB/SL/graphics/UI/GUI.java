...
public class GUI extends CheckBounds {
	...
	private transient Sprite HealthSprite = Sprite.HealthBar20;
	 private transient Sprite ManaSprite = Sprite.manabar20;
	  private transient Sprite StaminaSprite = Sprite.manabar20;
		...
		public AnimatedSprite healthbar = new AnimatedSprite(SpriteSheet.anim_hb, 32, 32, 21);
		public AnimatedSprite manabar = new AnimatedSprite(SpriteSheet.anim_mb, 32, 32, 21);
		public AnimatedSprite staminabar = new AnimatedSprite(SpriteSheet.anim_sb, 32, 32, 21);
		public AnimatedSprite expbar = new AnimatedSprite(SpriteSheet.anim_eb, 156, 32, 151);
	...
	public void renderName(Screen screen, String PersonNameGetter, int x, int y, int spacing, boolean fixed, boolean background, boolean font8) {
			int color = 0xFFFFFF;
			if (PersonNameGetter.equalsIgnoreCase("Nate")
					|| PersonNameGetter.equalsIgnoreCase("coinreturn1")) {
				colorMod++;
				if ((colorMod % 60) == 0) {
					colortime++;
					colorMod = 0;
			...
			} else if (PersonNameGetter.equalsIgnoreCase("Robbie")
					|| PersonNameGetter.equalsIgnoreCase("R0881E")
					|| PersonNameGetter.equalsIgnoreCase("GeekSquad2")) {
				color = 0x8899f7;
			} else if (PersonNameGetter.equalsIgnoreCase("Pascal")
					|| PersonNameGetter.equalsIgnoreCase("Nickavia")) {
				color = 0x0066FF;
			} else if (PersonNameGetter.equalsIgnoreCase("*[A]7381")
					|| PersonNameGetter.equalsIgnoreCase("AggressiveChairlift")) {
				color = 0x990000;
			} else if (PersonNameGetter.equalsIgnoreCase("Meg")
					|| PersonNameGetter.equalsIgnoreCase("Megan")) {
				color = 0xF0C4E5;
			...
			if (!font8) {
			font.render(x, y, spacing, color, PersonNameGetter, screen, fixed,
					background);
		} else {
			font8x8.render(x, y, spacing, color, PersonNameGetter, screen, fixed,
					background);
			}
		}
	
	
	public void renderString(Screen screen, String PersonNameGetter, int x, int y, int spacing, int color, boolean fixed, boolean background, boolean font8) {
		if (!font8) {
		font.render(x, y, spacing, color, PersonNameGetter, screen, fixed,
				background);
	} else {
	...
	public void renderMainMenu(Screen screen) {
		...
		fadeTime2++;
			...
			if (overSave == 1) {
				...
				font8x8.render(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, -2, 0xffffffff, "Level: " + tempLoadInfo.Lvl, screen, false, true, 7);
			} else if (overSave == 2) {
				if (tempLoadInfo == null) {
			...
			if (overTutorial) {
				font8x8.render(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, -2, 0xffffffff, "Run The Tutorial", screen, false, true, 7);
			}
...
//				Game.getGame().font.render(-6, 139, "X", Game.getGame().getScreen(), false, false);
//			}
//
//			if (overPerma) {
//				font8x8.render(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, -2, 0xffffffff, "On Death You Lose Everything", screen, false, true, 7);
//			}
	...
	public void renderDeath(Screen screen) {
		...
		screen.renderSheet(0, 0, SpriteSheet.Death, false);
	}
		if (overMenu) {
			screen.renderSheet(0, 0, SpriteSheet.overDeathMenu, false);
		}
		if (overQuit) {
			screen.renderSheet(0, 0, SpriteSheet.overDeathExit, false);
		}
	...
	public Sprite renderStaminaExperiment(Mob mob, int size) {
		...
		/*	if (mob.stamina < mob.maxstamina|| mob.incombat) {
				displayTimeS = 0;
	...
	public void renderPoints(Screen screen, Player player) {
		...
		if (player.skillPoints != 0 || player.inventoryEnabled || player.inPointMenu) {
			...
			if (this.checkBounds(146, 152, 13, 13, true, true)) {
				...
				//screen.drawFillRect(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, 80, 8, 0xff302B23, false);
				font8x8.render(Mouse.getX() * Game.scale >> Game.TILE_BIT_SHIFT, Mouse.getY() * Game.scale >> Game.TILE_BIT_SHIFT, -2, 0, "Points: " + player.skillPoints, screen, false, true, 1);
	...
	public void renderPointMenu(Screen screen, Player player) {
		...
		screen.renderSheet(82, 32, SpriteSheet.statPoints, false);
		//font8x8.render(83, 30, "Points: "+(int)player.skillPoints, screen, false, false);

		
		//Done Button
		if (this.checkBounds(182, 108, 23, 9, true, true)) {
		...
		//VIT
		font8x8.render(85, 52, -3, ""+(int)player.stat_base_VIT + "("+(int)player.stat_item_VIT+")", screen, false, false);
		if (this.checkBounds(108, 41, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_VIT += pointVal(player);
					player.skillPoints -= pointVal(player);
		...
		//WIS
		font8x8.render(128, 52, -3, ""+(int)player.stat_base_WIS + "("+(int)player.stat_item_WIS+")", screen, false, false);
		if (this.checkBounds(151, 41, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_WIS += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
		...
		//EDR
		font8x8.render(171, 52, -3, ""+(int)player.stat_base_EDR + "("+(int)player.stat_item_EDR+")", screen, false, false);
		if (this.checkBounds(193, 41, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_EDR += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
		...
		//ATC
		//screen.drawRect(108, 72, 15, 7, 0xff00FF00, false);
		font8x8.render(85, 82, -3, ""+(int)player.stat_base_ATC + "("+(int)player.stat_item_ATC+")", screen, false, false);
		if (this.checkBounds(108, 72, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_ATC += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
		...
		//DEF
		//screen.drawRect(151, 72, 15, 7, 0xff00FF00, false);
		font8x8.render(129, 82, -3, ""+(int)player.stat_base_DEF + "("+(int)player.stat_item_DEF+")", screen, false, false);
		if (this.checkBounds(151, 72, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_DEF += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
		...
		//MAT
		font8x8.render(171, 82, -3, ""+(int)player.stat_base_MAT + "("+(int)player.stat_item_MAT+")", screen, false, false);
		if (this.checkBounds(193, 72, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_MAT += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
		...
		//MDF
		//screen.drawRect(108, 103, 15, 7, 0xff00FF00, false);
		font8x8.render(85, 113, -3, ""+(int)player.stat_base_MDF + "("+(int)player.stat_item_MDF+")", screen, false, false);
		if (this.checkBounds(108, 103, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_MDF += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
		...
		font8x8.render(129, 113, -3, ""+(int)player.stat_base_AGI + "("+(int)player.stat_item_AGI+")", screen, false, false);
		if (this.checkBounds(151, 103, 15, 7, true, true)) {
			if (Mouse.getButton() == 1) {
				if(player.skillPoints > 0) {
					player.stat_base_AGI += pointVal(player);
					player.skillPoints -= pointVal(player);
					player.calcStat(false);
			buttonActivated();
				}
	...
	/*	if (Game.Mana >= 20) {
	screen.renderSprite(x, y, Sprite.manabar20, false);
	
}else if (Game.Mana == 19) {
	screen.renderSprite(x, y, Sprite.manabar19, false);
	
}else if (Game.Mana == 18) {
	screen.renderSprite(x, y, Sprite.manabar18, false);
	
}else if (Game.Mana == 17) {
	screen.renderSprite(x, y, Sprite.manabar17, false);
	
}else if (Game.Mana == 16) {
	screen.renderSprite(x, y, Sprite.manabar16, false);
	
}else if (Game.Mana == 15) {
	screen.renderSprite(x, y, Sprite.manabar15, false);
	
}else if (Game.Mana == 14) {
	screen.renderSprite(x, y, Sprite.manabar14, false);
	
}else if (Game.Mana == 13) {
	screen.renderSprite(x, y, Sprite.manabar13, false);
	
}else if (Game.Mana == 12) {
	screen.renderSprite(x, y, Sprite.manabar12, false);
	
}else if (Game.Mana == 11) {
	screen.renderSprite(x, y, Sprite.manabar11, false);
	
}else if (Game.Mana == 10) {
	screen.renderSprite(x, y, Sprite.manabar10, false);
	
}else if (Game.Mana == 9) {
	screen.renderSprite(x, y, Sprite.manabar9, false);
	
}else if (Game.Mana == 8) {
	screen.renderSprite(x, y, Sprite.manabar8, false);
	
}else if (Game.Mana == 7) {
	screen.renderSprite(x, y, Sprite.manabar7, false);
	
}else if (Game.Mana == 6) {
	screen.renderSprite(x, y, Sprite.manabar6, false);
	
}else if (Game.Mana == 5) {
	screen.renderSprite(x, y, Sprite.manabar5, false);
	
}else if (Game.Mana == 4) {
	screen.renderSprite(x, y, Sprite.manabar4, false);
	
}else if (Game.Mana == 3) {
	screen.renderSprite(x, y, Sprite.manabar3, false);
	
}else if (Game.Mana == 2) {
	screen.renderSprite(x, y, Sprite.manabar2, false);
	
}else if (Game.Mana == 1) {
	screen.renderSprite(x, y, Sprite.manabar1, false);
	
}else if (Game.Mana <= 0) {
	screen.renderSprite(x, y, Sprite.manabar0, false);
}		*/
